...
class FeedCommentListSerializer(serializers.ModelSerializer):
	class Meta:
		model = FeedComment
		fields = ('uuid', 'user', 'feed', 'text', 'created_at')

	def create(self, validated_data):
		request = self.context.get('request')
		user = request.user
		with transaction.atomic():
			return FeedComment.objects.create(user=user, **validated_data)


class FeedCommentLikeSerializer(serializers.ModelSerializer):
	class Meta:
		model = FeedCommentLike
		fields = ('uuid', 'user', 'feed_comment', 'created_at')

	def create(self, validated_data):
		request = self.context.get('request')
		user = request.user
		with transaction.atomic():
			return FeedCommentLike.objects.create(user=user, **validated_data)
...
class FeedLikeListSerializer(serializers.ModelSerializer):
	...
	def create(self, validated_data):
		...
		user = request.user
		with transaction.atomic():
			return FeedLike.objects.create(user=user, **validated_data)
